...
/* Check file extension */
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
        if (!$firstRow) {
            $password = password_hash(cleanData($conn, $data[7]), PASSWORD_DEFAULT);
            $random = $data[0].$data[1].rand();
            $sql = "INSERT INTO student ( `firstName`, `surName`, `middleName`,`post`, `email`,`address`,`S_id`,`password` ,`uniqid_id`,`date_of_birth`) VALUES ('" . $data[0] . "', '" . $data[1] . "', '" . $data[2] . "', '" . $data[3] . "','" . $data[4] . "','" . $data[5] . "','" . $data[6] . "','" . $password . "','" . $random . "','" . $data[8] . "')";
            if(mysqli_query($conn, $sql))
            {
           echo "<div class='mt-2 alert-success alert-sm p-2'> File upload into the database
            <script>$('#form-control-csv-resert-3')[0].reset()</script>
           <div>";
