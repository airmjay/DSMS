...
/* Check file extension */
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
        if (!$firstRow) {
            ...
            $sql = "INSERT INTO parent ( `firstName`, `surName`, `middleName`,`email`,`address`,`parent_id`,`password` ,`uniqid_id`) VALUES ('" . $data[0] . "', '" . $data[1] . "', '" . $data[2] . "', '" . $data[3] . "','" . $data[4] . "','" . $data[5] . "','" . $password . "','" . $random . "')";
            if(mysqli_query($conn, $sql))
            {
           echo "<div class='mt-2 alert-success alert-sm p-2'> File upload into the database!
           <script>$('#form-control-csv-resert-4')[0].reset()</script>
           </div>";
